...
export function Hero() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(
        g,
        { scale: 0.82, rotateZ: -4, opacity: 0.35, filter: 'blur(8px)' },
        {
          scale: 1,
          rotateZ: 0,
          opacity: 1,
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0 z-1 grid place-items-center">
        <div ref={ghost} className="ghost-word">
          GREATS
        </div>
      </div>
      ...
      <div
        ref={intro}
        className="parallax-header absolute top-24 left-5.5 sm:top-27.5 sm:left-9 lg:top-35 lg:left-16 flex flex-col gap-2 max-w-md z-5 pointer-events-none"
      >
      ...
      <div className="absolute left-5.5 right-5.5 bottom-5.5 sm:left-9 sm:right-9 sm:bottom-9 lg:left-16 lg:right-16 lg:bottom-16 flex flex-col sm:flex-row items-stretch sm:items-end justify-between gap-3 sm:gap-6 z-5">
        <div className="bg-paper border-4 border-ink px-5 py-3 sm:px-5.5 sm:pt-3 sm:pb-4 shadow-brutal-md flex flex-col gap-1.5 min-w-0 sm:min-w-55 max-w-md">
          <div
            ref={bottomTitle}
            className="font-display text-[28px] sm:text-3xl lg:text-4xl tracking-[-0.02em] leading-[1] text-ink uppercase"
          >
            The Goats.
          </div>
          <div
